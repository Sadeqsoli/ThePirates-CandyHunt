...
public class Level025 : MonoBehaviour
{
    ...
    private float currentTime = 45.99f;
    private const int levelUnlocked = 49;
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 5 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.coins > 25 &&
            gameLog.unitEnemyShipDistroyed >= 4 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
