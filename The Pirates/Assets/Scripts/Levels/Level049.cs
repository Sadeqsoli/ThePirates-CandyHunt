...
public class Level049 : MonoBehaviour
{
    ...
    private float currentTime = 60.99f;
    private const int levelUnlocked = 73;
    ...
    void Awake()
    {
        ...
        rainCoinSpawner.SetActive(false);
        isFinished = false;
    }//Awakeeeee
    ...
    private void Timer()
    {
        ...
        if (currentTime <= 0)
        {
            ...
            rainCoinSpawner.SetActive(true);
        }
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 7 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.coins > 40  &&
            gameLog.unitEnemyShipDistroyed >= 2 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
