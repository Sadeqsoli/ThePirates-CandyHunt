...
public class Level217 : MonoBehaviour
{
    ...
    private float currentTime = 175.99f;
    private const int levelUnlocked = 241;
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 10 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.unitEnemyShipDistroyed >= 10
            && gameLog.coins > 110 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
