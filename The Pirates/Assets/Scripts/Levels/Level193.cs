...
public class Level193 : MonoBehaviour
{
    ...
    private float currentTime = 150.99f;
    private const int levelUnlocked = 217;
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 10 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.unitEnemyShipDistroyed >= 9
            && gameLog.coins > 100 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
