...
public class Level288 : MonoBehaviour
{
    ...
    private float currentTime = 250.99f;
    private const int levelUnlocked = 288;
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 10 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.unitEnemyShipDistroyed >= 15
            && gameLog.coins > 150 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
