...
public class Level241 : MonoBehaviour
{
    ...
    private float currentTime = 200.99f;
    private const int levelUnlocked = 265;
    ...
    {
        Timer();
        if (gameLog.JunkCandyCrossed >= 10 && gameLog.motherEnemyShipDistroyed >= 3 && gameLog.unitEnemyShipDistroyed >= 11
            && gameLog.coins > 120 && isFinished == true && !OnlyOnec)
        {
            WinPanelPopup();
            OnlyOnec = true;
        }
