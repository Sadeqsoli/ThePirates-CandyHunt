...
public class ShipController : MonoBehaviour
{
    ...
    private void BallPooling(Vector3 pos)
    {
        bool find = false;
        ...
        if (find == false)
        {
            GameObject ball = Instantiate(bullet, pos, Quaternion.identity) as GameObject;
    ...
    private void Fire()
    {
        if (Time.time > fireRate + lastShot && gameController.HasBullet())
        {
            ...
            for (int i = 0; i <= choose; i++)
            {
                ...
                BallPooling(guns[i].transform.position);
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            collision.gameObject.GetComponent<CandyController>().EatCandy();
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("junkcandy"))
        {
            ...
            gameController.AddDestroyedItems("junkcandy");
            CheckHealth();
        }
    }
    ...
    private void CheckHealth()
    {
        if (Health <= 0)
        {
                ...
                gameController.AnotherChanceToWin();
                _health = InitHealth / 2; 
        }
